---
title: "Feature Engineering for MLB Batter Classification Models"
sidebarTitle: "Feature Engineering"
description: "How raw Statcast metrics are selected and transformed into feature sets for three Random Forest classifiers: performance, elite, and plate discipline."
---
...
All three models use an **80/20 train/test split** with `random_state=42` for reproducibility. The Elite and Discipline models also pass `stratify=y` to preserve the class distribution in both splits, which is critical given the class imbalance in `elite_hitter` and the uneven bin sizes in `clase_disciplina_home`.
